...
enum GuitarTuningsCatalog {
    ...
    static let halfStepDown: GuitarTuning = {
        ...
        return GuitarTuning(name: "Half Step Down (Eb Standard)", strings: shifted)
    }()
    ...
    static let wholeStepDown: GuitarTuning = {
        ...
        let shifted = base.enumerated().map { (idx, pair) in (names[idx], pair.1 * factor) }
        return GuitarTuning(name: "Whole Step Down (D Standard)", strings: shifted)
    }()
    ...
    static let dropB: GuitarTuning = {
        let baseStd: [(String, Float)] = [("E2", 82.41), ("A2", 110.00), ("D3", 146.83), ("G3", 196.00), ("B3", 246.94), ("E4", 329.63)]
        let factorOthers: Float = pow(2.0, -4.0/12.0)
        let factorLow: Float = pow(2.0, -7.0/12.0)
        let shifted: [(String, Float)] = [
            ("B1", baseStd[0].1 * factorLow),
            ("F#2", baseStd[1].1 * factorOthers),
            ("B2", baseStd[2].1 * factorOthers),
            ("E3", baseStd[3].1 * factorOthers),
            ("G#3", baseStd[4].1 * factorOthers),
            ("C#4", baseStd[5].1 * factorOthers)
        ]
        return GuitarTuning(name: "Drop B", strings: shifted)
    }()
    ...
    static let openA: GuitarTuning = .init(
        name: "Open A",
        strings: [("E2", 82.41), ("A2", 110.00), ("E3", 164.81), ("A3", 220.00), ("C#4", 277.18), ("E4", 329.63)]
    )

    static let doubleDropD: GuitarTuning = .init(
        name: "Double Drop D",
        strings: [("D2", 73.42), ("A2", 110.00), ("D3", 146.83), ("G3", 196.00), ("B3", 246.94), ("D4", 293.66)]
    )

    static let dadgad: GuitarTuning = .init(
        name: "DADGAD",
        strings: [("D2", 73.42), ("A2", 110.00), ("D3", 146.83), ("G3", 196.00), ("A3", 220.00), ("D4", 293.66)]
    )

    static let openC: GuitarTuning = .init(
        name: "Open C",
        strings: [("C2", 65.41), ("G2", 98.00), ("C3", 130.81), ("G3", 196.00), ("C4", 261.63), ("E4", 329.63)]
    )

    static let all: [GuitarTuning] = [
        standardE, dropD, halfStepDown, wholeStepDown, dropC, dropB,
        openD, openE, openG, openA, doubleDropD, dadgad, openC
    ]
}
